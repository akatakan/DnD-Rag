...
export default function DMConsole({ snapshot, token, onError }: { snapshot: Snapshot; token: string; onError: (value: string) => void }) {
  ...
  return (
    <main className="dm-layout">
      ...
      <aside className="dm-left">
        <section className="invite-panel"><span className="eyebrow">Davet kodu</span><div><strong>{snapshot.game.invite_code}</strong><button className="icon-button" title="Kodu kopyala" onClick={() => navigator.clipboard.writeText(snapshot.game.invite_code || "")}><Copy size={17} /></button></div></section>
